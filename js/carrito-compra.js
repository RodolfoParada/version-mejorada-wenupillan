class CarritoCompras extends HTMLElement {
  constructor() {
    ...
    this._carritoActual = JSON.stringify(localStorage.getItem("carrito")); // Estado inicial
  }
  ...
  connectedCallback() {
    ...
    // También actualizar entre pestañas
    ...
    // Detectar cambios de localStorage cada 500ms (en esta misma pestaña)
    this._interval = setInterval(() => {
      const actual = JSON.stringify(localStorage.getItem("carrito"));
  ...
  disconnectedCallback() {
    clearInterval(this._interval); // limpiar cuando se quite del DOM
  }
...
actualizarCarrito() {
  ...
  // Crear contenedor de subtotales
...
subtotalEl.innerHTML = `
  ...
  <div class="d-flex align-items-center gap-1">
    ...
    <span id="contador-${nombre}" style="min-width: 20px; text-align: center;">${cantidad}</span>
...
subtotalesContainer.querySelectorAll("button").forEach((btn) => {
  ...
  eliminarProducto(index) {
    ...
    this._carritoActual = JSON.stringify(localStorage.getItem("carrito")); // actualizar cache
    this.actualizarCarrito();
  }
